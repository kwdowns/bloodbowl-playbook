...
- **`src/lib/rules/`** — pure functions over `FieldedPlayer[]`, no Vue/Pinia imports; this is where all game math lives and where unit tests are (`__tests__/`). `blocks.ts` computes assists (Guard-aware) and dice count/chooser; `blockDice.ts` computes the probability distribution of the *applied* block result assuming the chooser picks the best die for their side — "best" is derived from `resultEffects()`, which factors in Block/Dodge/Tackle; `tackleZones.ts` builds per-square zone-count grids; `dodge.ts` computes dodge target numbers and re-roll odds; `movement.ts` computes move-planner paths: per-step dodge/rush assessments, exact path odds with once-per-turn Dodge / Sure Feet re-rolls, and a best-chance reachability search; `pass.ts` holds the `RANGE_CHART` lookup (indexed by the throw vector's `[|Δrow|][|Δcolumn|]`), pass/catch/interference odds and ball-landing probability maps; `throwTeammate.ts` covers Throw Team-mate incl. the Really Stupid / Always Hungry gates.
...
- Tailwind v4 (CSS-first): there is no `tailwind.config.js` — `src/assets/main.css` does `@import 'tailwindcss'` and sources are auto-detected. PostCSS uses `@tailwindcss/postcss` (see `postcss.config.js`). Add custom theme values via `@theme` in CSS, not a JS config.
